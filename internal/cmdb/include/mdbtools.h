/* Configured copy for the cgo bridge. Keep declarations in sync with mdbtools/include/mdbtools.h.in. */
#ifndef _mdbtools_h_
#define _mdbtools_h_
...
enum { MDB_PAGE_DB = 0, MDB_PAGE_DATA, MDB_PAGE_TABLE, MDB_PAGE_INDEX, MDB_PAGE_LEAF, MDB_PAGE_MAP };
enum { MDB_VER_JET3 = 0, MDB_VER_JET4 = 0x01, MDB_VER_ACCDB_2007 = 0x02, MDB_VER_ACCDB_2010 = 0x03, MDB_VER_ACCDB_2013 = 0x04, MDB_VER_ACCDB_2016 = 0x05, MDB_VER_ACCDB_2019 = 0x06 };
enum { MDB_FORM = 0, MDB_TABLE, MDB_MACRO, MDB_SYSTEM_TABLE, MDB_REPORT, MDB_QUERY, MDB_LINKED_TABLE, MDB_MODULE, MDB_RELATIONSHIP, MDB_UNKNOWN_09, MDB_UNKNOWN_0A, MDB_DATABASE_PROPERTY, MDB_ANY = -1 };
enum { MDB_BOOL = 0x01, MDB_BYTE = 0x02, MDB_INT = 0x03, MDB_LONGINT = 0x04, MDB_MONEY = 0x05, MDB_FLOAT = 0x06, MDB_DOUBLE = 0x07, MDB_DATETIME = 0x08, MDB_BINARY = 0x09, MDB_TEXT = 0x0a, MDB_OLE = 0x0b, MDB_MEMO = 0x0c, MDB_REPID = 0x0f, MDB_NUMERIC = 0x10, MDB_COMPLEX = 0x12 };
enum { MDB_OR = 1, MDB_AND, MDB_NOT, MDB_EQUAL, MDB_GT, MDB_LT, MDB_GTEQ, MDB_LTEQ, MDB_LIKE, MDB_ISNULL, MDB_NOTNULL, MDB_ILIKE, MDB_NEQ };
...
enum { MDB_DEBUG_LIKE = 0x0001, MDB_DEBUG_WRITE = 0x0002, MDB_DEBUG_USAGE = 0x0004, MDB_DEBUG_OLE = 0x0008, MDB_DEBUG_ROW = 0x0010, MDB_DEBUG_PROPS = 0x0020, MDB_USE_INDEX = 0x0040, MDB_NO_MEMO = 0x0080 };
typedef enum { MDB_BRACES_4_2_2_8, MDB_NOBRACES_4_2_2_2_6 } MdbUuidFormat;

#define mdb_is_logical_op(x) (x == MDB_OR || x == MDB_AND || x == MDB_NOT)
#define mdb_is_relational_op(x) (x == MDB_EQUAL || x == MDB_GT || x == MDB_LT || x == MDB_GTEQ || x == MDB_LTEQ || x == MDB_NEQ || x == MDB_LIKE || x == MDB_ILIKE || x == MDB_ISNULL || x == MDB_NOTNULL)

enum { MDB_ASC, MDB_DESC };
enum { MDB_IDX_UNIQUE = 0x01, MDB_IDX_IGNORENULLS = 0x02, MDB_IDX_REQUIRED = 0x08 };
enum { MDB_SHEXP_DROPTABLE = 1 << 0, MDB_SHEXP_CST_NOTNULL = 1 << 1, MDB_SHEXP_CST_NOTEMPTY = 1 << 2, MDB_SHEXP_COMMENTS = 1 << 3, MDB_SHEXP_DEFVALUES = 1 << 4, MDB_SHEXP_INDEXES = 1 << 5, MDB_SHEXP_RELATIONS = 1 << 6, MDB_SHEXP_BULK_INSERT = 1 << 7 };
#define MDB_SHEXP_DEFAULT (MDB_SHEXP_CST_NOTNULL | MDB_SHEXP_COMMENTS | MDB_SHEXP_INDEXES | MDB_SHEXP_RELATIONS)
enum { MDB_BINEXPORT_STRIP, MDB_BINEXPORT_RAW, MDB_BINEXPORT_OCTAL, MDB_BINEXPORT_HEXADECIMAL, MDB_EXPORT_ESCAPE_CONTROL_CHARS = (1 << 4) };
...
typedef struct { char *name; unsigned char needs_precision; unsigned char needs_scale; unsigned char needs_byte_length; unsigned char needs_char_length; } MdbBackendType;
typedef struct {
...
typedef struct { gboolean collect; unsigned long pg_reads; } MdbStatistics;
typedef struct {
	FILE *stream;
	gboolean writable;
	guint32 jet_version;
...
typedef struct {
	...
	char *backend_name;
	MdbTableDef *relationships_table;
...
typedef struct { gchar *name; GHashTable *hash; } MdbProperties;
typedef union { int i; double d; char s[256]; } MdbAny;
...
typedef struct {
	...
	int cur_value_start;
	int cur_value_len;
	unsigned char cur_value_is_null;
...
typedef struct { guint32 pg; int start_pos; int offset; int len; int rc; guint16 idx_starts[2000]; unsigned char cache_value[256]; } MdbIndexPage;
typedef int (*MdbSargTreeFunc)(MdbSargNode *, gpointer data);
#define MDB_MAX_INDEX_DEPTH 10
typedef struct { int cur_depth; guint32 last_leaf_found; int clean_up_mode; MdbIndexPage pages[MDB_MAX_INDEX_DEPTH]; } MdbIndexChain;
...
typedef struct { char name[MDB_MAX_OBJ_NAME + 1]; } MdbColumnProp;
typedef struct { void *value; int siz; int start; unsigned char is_null; unsigned char is_fixed; int colnum; int offset; } MdbField;
typedef struct { int op; MdbAny value; } MdbSarg;
