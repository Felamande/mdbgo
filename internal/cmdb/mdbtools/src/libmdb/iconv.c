...
#if !defined(HAVE_ICONV) && (defined(_WIN32) || defined(WIN32) || defined(_WIN64) || defined(WIN64) || defined(WINDOWS))
#include <windows.h>
#endif
...
static size_t unicode2ascii_locale(mdb_locale_t locale, const char *in_ptr, size_t len_in, char *dest, size_t dlen) {
    ...
    size_t wlen = len_in / 2;
    int cbMultiByte = (len_out > INT_MAX) ? INT_MAX : (int)len_out;
    int cchWideChar = (wlen > INT_MAX) ? INT_MAX : (int)wlen;
    (void)locale;

    while (cchWideChar > 0) {
        count = WideCharToMultiByte(CP_UTF8, 0, w, cchWideChar, dest, cbMultiByte, NULL, NULL);
